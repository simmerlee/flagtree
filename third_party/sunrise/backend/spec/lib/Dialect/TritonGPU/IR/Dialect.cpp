...
LogicalResult DotOperandEncodingAttr::verify(
  ...
  if (auto parentAttr = mlir::dyn_cast<SunriseMmaEncodingAttr>(parent)) {
    return success();
  }
...
Attribute SunriseMmaEncodingAttr::parse(AsmParser &parser, Type type) {
  ...
  for (const NamedAttribute &attr : dict) {
    ...
    if (attr.getName() == "CTAsPerCGA") { if (parseIntArrayAttr(parser, attr, CTAsPerCGA.emplace(), "CTAsPerCGA").failed()) {return {};} }
    if (attr.getName() == "CTASplitNum") { if (parseIntArrayAttr(parser, attr, CTASplitNum.emplace(), "CTASplitNum").failed()){return {};} }
    if (attr.getName() == "CTAOrder") { if (parseIntArrayAttr(parser, attr, CTAOrder.emplace(), "CTAOrder").failed()) {return {};} }
  ...
  return parser.getChecked<SunriseMmaEncodingAttr>(
      parser.getContext(), versionMajor, versionMinor, warpsPerCTA, *CTALayout, outLayout, inputElemBitWidth, outputElemBitWidth);
}
...
void SunriseMmaEncodingAttr::print(AsmPrinter &printer) const {
  ...
  // printer << ", order = " << sv2str(getOrder(*this))
  //         << ", threadsPerWarp = " << sv2str(getThreadsPerWarp())
  //         << ", threadOrder = " << sv2str(getThreadOrder())
  //         << ", sizePerThread = " << sv2str(getSizePerThread())
  //         << ", shapePerCTATile = " << sv2str(getShapePerCTATile())
...
// sunrise_mma encoding
//===----------------------------------------------------------------------===//
SmallVector<unsigned> SunriseMmaEncodingAttr::getRepOrder() const {
  return getMatrixOrder(getRank(), /*rowMajor*/ true);
}

SmallVector<unsigned>
SunriseMmaEncodingAttr::getRepOrderForOperand(int opIdx) const {
  return getOrderForDotOperand(opIdx, getRank(), /*kContig*/ true);
}

SmallVector<unsigned>
SunriseMmaEncodingAttr::getInstrShapeForOperand(unsigned opIdx) const {
  // A, B矩阵每个warp加载的尺寸，只支持行主序
...
SmallVector<unsigned>
SunriseMmaEncodingAttr::getShapePerCTATileForOperand(unsigned opIdx) const {
  // CTA内所有warp执行一次mma操作对应输入tensor的尺寸
  ...
  if (opIdx == 0) {
    ret = {shapePerCTATile[0], k};
  } else {
    ret = {k, shapePerCTATile[1]};
  }
  return ret;
}

SmallVector<unsigned> SunriseMmaEncodingAttr::getShapePerCTATile() const {
  // CTA内所有warp执行一次mma操作的结果的尺寸
  auto warpsPerCTA = getWarpsPerCTA();
  assert(warpsPerCTA.size() == 2);
  SmallVector<unsigned> shapePerCTATile(2);
  shapePerCTATile[0] = warpsPerCTA[0] * 8;
  shapePerCTATile[1] = warpsPerCTA[1] * 8;
  return shapePerCTATile;
}

SmallVector<unsigned> SunriseMmaEncodingAttr::getCTAsPerCGA() const {
  return SmallVector<unsigned>(getCTALayout().getCTAsPerCGA());
}
SmallVector<unsigned> SunriseMmaEncodingAttr::getCTAOrder() const {
  return SmallVector<unsigned>(getCTALayout().getCTAOrder());
}
SmallVector<unsigned> SunriseMmaEncodingAttr::getCTASplitNum() const {
  return SmallVector<unsigned>(getCTALayout().getCTASplitNum());
}
