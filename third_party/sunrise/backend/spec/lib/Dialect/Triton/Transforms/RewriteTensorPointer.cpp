...
public:
  ...
  unsigned getRowStrideParamIndex() const { return rowStrideParamIndex; }

  bool getHasRowStride() const { return hasRowStride; }
  ...
  Operation *rewriteMakeTensorPtrOp(OpBuilder &builder,
    ...
    rewritedInfo[op.getResult()] =
        RewritedInfo(op.getBase(), op.getShape(), op.getStrides(), i64Offsets,
                     tensorType.getShape(), hasRowStride, rowStrideParamIdx);
