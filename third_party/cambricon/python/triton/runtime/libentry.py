...
# NOTE: Do not modify this file unless there is a bug.
import hashlib
import inspect
import math
...
import triton
from .autotuner import Autotuner, Heuristics
...
TRITON_ENABLE_DISK_CACHE = os.getenv("TRITON_ENABLE_DISK_CACHE", "1") == "1"
...
class Cache(object):
    ...
    def __init__(self, table_name: str, conns: Connections, *args, **kwargs) -> Cache:
        ...
        self.py2sql: Dict[type, str] = {
            bool: "BOOLEAN", int: "INTEGER", float: "DOUBLE", str:
            "VARCHAR(16)",  # it often stores string values like 'torch.float16', so 16 would be long enough
        }

    @staticmethod
    def build_key_dict(key: Tuple[Union[int, float, str], ...]) -> Dict[str, Any]:
        return {f"key_{k}": v for k, v in enumerate(key) if isinstance(v, (int, float, str))}

    @staticmethod
    def build_config_dict(config: triton.Config) -> Dict[str, Any]:
        return {k: v for k, v in config.all_kwargs().items() if isinstance(v, (int, float, str))}
...
class ConfigCache(Cache):
    ...
    def __init__(self, table_name: str, conns: Connections, *args, **kwargs) -> ConfigCache:
        super().__init__(table_name, conns, *args, **kwargs)
        self.config_signature: inspect.Signature = inspect.signature(triton.Config)
        self.dict_cache: Dict[Tuple[Union[int, float, str], ...],
                              triton.Config] = {}  # this dict is used to cache some results in the memory
        ctx = multiprocessing.get_context()
        self.lock = ctx.Lock()
        self.names: List[str] = [
            name for _, name, _, _, _, _ in self.conn.execute(f"PRAGMA table_info({self.table_name});").fetchall()
        ]
        self.create_sql: Optional[
            str] = None  # if the corresponding sql instruction is None, meaning the table is not ready, we need to flush it
    ...
    def __setitem__(self, key: Tuple[Union[int, float, str], ...], config: triton.Config):
        self.set(key, config)

    @property
    def knames(self) -> Iterator[str]:
        return filter(lambda name: name.startswith("key_"),
                      self.names)  # if the column name starts with "key_", it should be a key defined by the libtuner

    @property
    def cnames(self) -> Iterator[str]:
        return filter(lambda name: not name.startswith("key_"),
                      self.names)  # otherwise, it should be a parameter in the config
        ...
        with self.lock:
            ...
            if self.create_sql is None:
                self.create_sql = "CREATE TABLE IF NOT EXISTS {} ({});".format(
                    self.table_name,
                    ",\n".join([f"{k} {self.py2sql[type(v)]}" for k, v in queries.items()] +
    ...
    def __getitem__(self, config: triton.Config) -> Optional[List[float]]:
        with self.lock:
            ...
            if self.select_sql is None:
                ...
                self.select_sql = (f"SELECT p50, p20, p80 FROM {self.table_name} WHERE {where};")
    ...
    def __setitem__(self, config: triton.Config, benchmark: List[float]) -> None:
        with self.lock:
            ...
            if not self.insert_sql:
                self.insert_sql = "INSERT OR REPLACE INTO {} VALUES ({});".format(self.table_name,
                                                                                  ", ".join("?" for _ in values))
            self.conn.execute(self.insert_sql, values)
        self.conn.commit()

    def build_query(self, config: triton.Config) -> Dict[str, Any]:
        queries: Dict[str, Any] = self.build_key_dict(self.key) | self.build_config_dict(config)
        if self.create_sql is None:
            self.create_sql = "CREATE TABLE IF NOT EXISTS {} ({});".format(
                self.table_name,
                ",\n".join([f"{k} {self.py2sql[type(v)]}" for k, v in queries.items()] +
...
class LibCache(object):
    ...
    def __init__(self, enable_disk_cache: bool):
        ...
        self.volumn: Dict = {}
        cache_file_name = f"TunedConfig_{major_version}_{minor_version}.db"
        self.cache_path = ((config_cache_dir() / cache_file_name) if enable_disk_cache else ":memory:")
        ctx = multiprocessing.get_context()
        self.bench_lock = ctx.Lock()
        self.config_lock = ctx.Lock()
        self.conns: Connections = Connections(self.cache_path)
        self.config_cache_pool: Dict[str, ConfigCache] = {}
        self.benchmark_cache_pool: Dict[Tuple[str, Tuple[Union[int, float, str], ...]], BenchmarkCache] = {}
    ...
    def __getitem__(self, key: Union[str, Tuple[Union[int, float, str], ...]]) -> Union[BenchmarkCache, ConfigCache]:
    ...
    def get_benchmark(self, table: str, key: Tuple[Union[int, float, str], ...]) -> BenchmarkCache:
...
# Lazy mode of cache for concurrent multi-process.
...
def LazyCache():
    ...
    if _cache_instance is None:
        with _cache_lock:
            if _cache_instance is None:
                _cache_instance = LibCache(TRITON_ENABLE_DISK_CACHE)
...
class LibTuner(Autotuner):
    ...
    def __init__(
        ...
        assert len(strategy) == len(
            self.keys), f"the length of strategy {len(strategy)} must match the length of keys {len(self.keys)}"
        strategy: List[Callable[[Any], Any]] = [LibTuner.get_strategy(s) if isinstance(s, str) else s for s in strategy]
    ...
    def kernel_hash(self):
        return hashlib.md5(f"{self.cache_key}{self.configs_hash}".encode("utf-8")).hexdigest()

    @cached_property
    def configs_hash(self):
        return hashlib.md5(",".join(map(lambda config: str(config), self.configs)).encode("utf-8")).hexdigest()
    ...
    def policy(
        ...
        raise NotImplementedError(f"`policy` isn't implemented in {self.__class__.__name__}")
    ...
    @staticmethod
    def register_policy(name: str, ) -> Type[LibTuner]:
        ...
        def decorator(policy_impl: Callable[
            [
    ...
    def register_strategy(name: str):
        ...
        def decorator(strategy: Union[Callable[[Any], Any], List[Callable[[Any], Any]]], ):
...
def default_policy(
    ...
    """
    timings: Dict[triton.Config, float] = {config: bench_fn(config) for config in configs}
    best_config: triton.Config = min(timings, key=timings.get)
    return best_config, timings
...
def libtuner(
    ...
    use_cuda_graph=False,
    do_bench=None,
    strategy: Union[str, Callable[[Any], Any], List[Union[str, Callable[[Any], Any]]]] = "default",
...
class LibEntry(KernelInterface):
