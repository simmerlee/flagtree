    ...
    def materialize_pointers(self, boundary_check):
        ...
        for dim in range(len(tensor_shape)):
            ...
            if dim in boundary_check:
                ...
                masks = np.logical_and(masks, off >= 0)
...
def _get_slice_list(offset_vals, size_vals, step_vals):
    offsets = []
    for offset in offset_vals:
        assert isinstance(offset, TensorHandle) and offset.data.shape == (1, )
        offsets.append(offset.data[0])
    sizes = [_constexpr_to_value(size) for size in size_vals]
    steps = [_constexpr_to_value(size) for size in step_vals]
    slice_list = [slice(offset, offset + size * step, step) for offset, size, step in zip(offsets, sizes, steps)]
    return slice_list


def _hex_formatter(x):
    if isinstance(x, np.float16):
        return f"0x{x.view(np.uint16):04x}"
    elif isinstance(x, np.float32):
        return f"0x{x.view(np.uint32):08x}"
    else:
        return f"0x{x:02x}"
...
class InterpreterBuilder:
    ...
    def create_empty_tensor(self, shape, type):
        return TensorHandle(np.empty(shape=shape, dtype=_get_np_dtype(type)), type)
    ...
    def create_masked_load(self, ptrs, mask, other, cache_modifier, eviction_policy, is_volatile):
        ...
        nullptr_indices = np.argwhere(ptrs.data == 0 & ((mask.data) if isinstance(mask.data, np.ndarray) else mask))
        if len(nullptr_indices) > 0:
            raise RuntimeError(f"A null pointer was encountered in load operation.")
    ...
    def create_masked_store(self, ptrs, value, mask, cache_modifier, eviction_policy):
        nullptr_indices = np.argwhere(ptrs.data == 0 & ((mask.data) if isinstance(mask.data, np.ndarray) else mask))
        if len(nullptr_indices) > 0:
            raise RuntimeError(f"A null pointer was encountered in store operation.")
    ...
    def create_flip(self, src, dims):
        return TensorHandle(np.flip(src.data, dims), src.dtype.scalar)
    ...
    def create_bitcast_ext(self, src, dst_type):
        return TensorHandle(src.data.view(_get_np_dtype(dst_type)), dst_type.scalar)
    ...
    def create_pow(self, lhs, rhs):
        rhs_dtype = rhs.data.dtype
        if rhs_dtype == np.int32:
            rhs.data = rhs.data.astype(np.float32)
        return self.binary_op(lhs, rhs, np.power)
    ...
    def create_scatter(self, dst, src, indices, axis):
        dst_slice = tuple([slice(0, size) for size in indices.data.shape])
        dst_ = dst.data[dst_slice]
        np.put_along_axis(dst_, indices.data, src.data, axis=axis)
        dst.data[dst_slice] = dst_
        return TensorHandle(dst.data, dst.dtype.scalar)
    ...
    def compute_constant_positive_delta(self, start, stop):
        # Numpy index should be i32 type.
        return int(stop.data[0] - start.data[0])

    def create_extract_slice(self, input, offsets, sizes, steps, dst_shape):
        slice_list = _get_slice_list(offsets, sizes, steps)
        return TensorHandle(input.data[tuple(slice_list)].reshape(dst_shape), input.dtype)

    def create_insert_slice(self, src, dst, offsets, sizes, steps):
        slice_list = _get_slice_list(offsets, sizes, steps)
        slice_data = dst.data[tuple(slice_list)]
        # Here we cannot use slice_data = src.data.reshape(slice_data.shape), as
        # it just change the reference of slice_data to src.
        dst.data[tuple(slice_list)] = src.data.reshape(slice_data.shape)
        return TensorHandle(dst.data, dst.dtype)

    def create_extract(self, input, indices):
        indices = [indice.data[0] for indice in indices]
        return TensorHandle(input.data[tuple(indices)], input.dtype)

    def create_insert(self, src, dst, indices):
        indices = [indice.data[0] for indice in indices]
        dst.data[tuple(indices)] = src.data
        return TensorHandle(dst.data, dst.dtype)
    ...
    def create_print(self, prefix, hex, values, isSigned):
        ...
        if prefix:
            msg += f" {prefix}"
        if hex:
            np.set_printoptions(formatter={'all': lambda x: _hex_formatter(x)})
        for value in values:
    ...
    def get_all_ones_value(self, type):
        ...
        if "int" in np_type.name or np_type.name == 'bool':
...
class ReduceOps(ReduceScanOpIneterface):
    ...
    def generic_reduce(self, input):
        ...
        return ret[0] if self.return_tensor else tuple(ret)
...
class ScanOps(ReduceScanOpIneterface):
    ...
    def apply_impl(self, input):
        ...
        return self.return_tensor and ret[0] or tuple(ret)
