...
class ASTFunction:
    ...
    def __init__(self, ret_types, arg_types, constexprs, constants, attrs):
        ...
        self.constants = constants | constexprs
        self.attrs = attrs
...
class CodeGenerator(ast.NodeVisitor):
    ...
    def visit_Return(self, node):
        ...
        def decay(value):
            if isinstance(value, language.tuple):
                ...
                return language.semantic.to_tensor(value, self.builder)
            return value
    ...
    def assignTarget(self, target, value):
        ...
        if isinstance(target, ast.Tuple):
            ...
            for i, name in enumerate(target.elts):
                self.assignTarget(name, value.values[i])
            return
    ...
    def visit_AugAssign(self, node):
        if isinstance(node.target, ast.Subscript):
            ...
            lhs = ast.Subscript(node.target.value, node.target.slice, ctx=ast.Load())
        else:
            name = node.target.id
    ...
    def visit_Subscript_Store(self, node, value):
        ...
        if isinstance(lhs, language.tuple):
            lhs.__setitem__(slices, value)
        elif _is_triton_tensor(lhs):
            dst = lhs.__setitem__(slices, value, _builder=self.builder)
            self.set_value(node.value.id, dst)
        else:
            assert False, f"Unsupported Subscript Store type {type(lhs)}"
    ...
    def visit_Subscript(self, node):
        ...
        if _is_triton_tensor(lhs):
            if node.ctx.__class__.__name__ == "Load":
                ...
                assert node.ctx.__class__.__name__ == "Store"
                return TensorSlicer(node.value.id, slices)
        return lhs[slices]
...
def ast_to_ttir(fn, specialization, context, options, codegen_fns, module_map):
    ...
    all_constants = {
        ...
        if not (isinstance(key, tuple) and len(key) > 1 and (key[0], ) in all_constants)
    }
    ...
    file_name, begin_line = get_jit_fn_file_line(fn)
    prototype = ASTFunction([], arg_types, constexprs, all_constants | constexprs, fn_attrs)
