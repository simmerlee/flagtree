...
def check_dtype_support(builder, dtype):
    if builder.options.backend_name in ['mlu', 'interpreter']:
        if dtype.is_fp64():
            raise RuntimeError(f'MLU unsupported floating-point type {dtype}')
        if dtype.is_block():
            return check_dtype_support(builder, dtype.scalar)
        if dtype.is_ptr():
            return check_dtype_support(builder, dtype.element_ty)
...
def computation_type_impl(a_ty: tl.dtype, a_is_scalar: bool, b_ty: tl.dtype, b_is_scalar: bool, div_or_mod: bool,
                          increase_bit_width: bool) -> tl.dtype:
...
def binary_op_type_checking_impl(lhs: tl.tensor | numbers.Number, rhs: tl.tensor | numbers.Number, builder: ir.builder,
                                 ...
                                 increase_bit_width=False) -> Tuple[tl.tensor, tl.tensor]:
...
def truediv(input: tl.tensor | numbers.Number, other: tl.tensor | numbers.Number, builder: ir.builder) -> tl.tensor:
    increase_bit_width = not builder.options.allow_half_div
    input, other = binary_op_type_checking_impl(input, other, builder, False, False, True, True, increase_bit_width)
    input_scalar_ty = input.type.scalar
    other_scalar_ty = other.type.scalar
    # float / int
...
def floordiv(input: tl.tensor | numbers.Number, other: tl.tensor | numbers.Number, builder: ir.builder) -> tl.tensor:
    input, other = binary_op_type_checking_impl(input, other, builder, False, False, True, True, True)
    input_scalar_ty = input.type.scalar
    other_scalar_ty = other.type.scalar
...
def fdiv(input: tl.tensor | numbers.Number, other: tl.tensor | numbers.Number, ieee_rounding: bool,
    ...
    increase_bit_width = not builder.options.allow_half_div
    input, other = binary_op_type_checking_impl(input, other, builder, False, False, False, True, increase_bit_width)
...
def empty(shape: List[int], dtype: tl.dtype, builder: ir.builder) -> tl.tensor:
    if isinstance(dtype, tl.constexpr):
        dtype = dst_ty.value
    if dtype is None:
        raise ValueError("dtype must be specified")
    if len(shape) == 0:
        return tl.tensor(builder.get_null_value(dtype.to_ir(builder)), dtype)
    ret_ty = tl.block_type(dtype, shape)
    return tl.tensor(builder.create_empty_tensor(shape, dtype.to_ir(builder)), ret_ty)
...
def extract_slice(input: tl.tensor, offsets: list[tl.tensor], sizes: list[tl.constexpr], strides: list[tl.constexpr],
                  dst_shape: list[tl.constexpr], builder: ir.builder) -> tl.tensor:
    rank = len(sizes)
    assert len(offsets) == rank
    # Extract scalar value when shape can be empty ([]).
    if not dst_shape:
        ret_type = input.type.scalar
        return tl.tensor(builder.create_extract(input.handle, offsets), ret_type)
    ret_type = tl.block_type(input.type.scalar, dst_shape)
    return tl.tensor(builder.create_extract_slice(input.handle, offsets, sizes, strides, dst_shape), ret_type)


def insert_slice(src: tl.tensor, dst: tl.tensor, offsets: list[tl.tensor], sizes: list[tl.constexpr],
                 strides: list[tl.constexpr], builder: ir.builder) -> tl.tensor:
    rank = len(sizes)
    assert len(offsets) == rank
    # Insert scalar value when shape can be empty ([]).
    if not src.type.is_block():
        return tl.tensor(builder.create_insert(src.handle, dst.handle, offsets), dst.type)
    return tl.tensor(builder.create_insert_slice(src.handle, dst.handle, offsets, sizes, strides), dst.type)
...
def stack(a: tl.tensor, b: tl.tensor, axis: int, builder: ir.builder) -> tl.tensor:
    # Ensure that `a` and `b` have the same shape and dtype
    assert a.shape == b.shape, "all stack inputs must have the same shape"

    a, b = binary_op_type_checking_impl(a, b, builder, True, True)

    # If axis is None, default to 0
    if axis is None:
        axis = 0

    # Handle negative axis values by converting them to positive indices
    ori_axis = axis
    if axis < 0:
        axis += len(a.shape) + 1
    ...
    if axis > len(a.shape) or axis < 0:
        raise ValueError("axis " + str(ori_axis) + " is out of bounds for "
                         "array of dimension " + str(len(a.shape) + 1))

    # Compute the resulting shape after stacking
    ret_shape = [tl._constexpr_to_value(x) for x in a.shape]
    ret_shape.insert(axis, 2)

    # Create an empty tensor to hold the result
    ret = empty(ret_shape, dtype=a.dtype, builder=builder)

    # Calculate size and strides for insertion
    size = [tl._constexpr_to_value(x) for x in a.shape]
    size.insert(axis, 1)
    strides = [1] * len(ret_shape)

    # Insert tensor `a` at the specified axis
    a_offset = [0] * len(ret_shape)
    a_offset = _convert_to_ir_values(builder, a_offset, require_i64=False)
    ret = insert_slice(a, ret, a_offset, size, strides, builder=builder)

    # Insert tensor `b` at the specified axis
    b_offset = [0] * len(ret_shape)
    b_offset[axis] = 1
    b_offset = _convert_to_ir_values(builder, b_offset, require_i64=False)
    ret = insert_slice(b, ret, b_offset, size, strides, builder=builder)
...
def bitcast(input: tl.tensor, dst_ty: tl.dtype, builder: ir.builder) -> tl.tensor:
    ...
    if src_bits != dst_bits:
        if builder.options.backend_name != "mlu":
            raise ValueError("Cannot bitcast data-type of size " + str(src_bits) + " to "
                             "data-type of size " + str(dst_bits))
        from triton._C.libtriton import mlu
        if src_sca_ty == tl.int1 or dst_sca_ty == tl.int1:
            raise ValueError("Unsupported bitcast type: i1.")
        shape = src_ty.shape
        if src_bits < dst_bits:
            multiple = dst_bits // src_bits
            if shape[-1] % multiple != 0:
                raise ValueError(f"When transitioning from bit width {src_bits} to bit width {dst_bits}, "
                                 f"the lowest dimension of the input needs to be divided by "
                                 f"{multiple}.")
            shape[-1] = shape[-1] // multiple
        else:
            multiple = src_bits // dst_bits
            shape[-1] = shape[-1] * multiple
        dst_ty = tl.block_type(dst_ty.scalar, shape)
        return tl.tensor(builder.create_bitcast_ext(input.handle, dst_ty.to_ir(builder)), dst_ty)
...
def cast(input: tl.tensor, dst_ty: tl.dtype, builder: ir.builder,
    ...
    src_sca_ty = src_ty.scalar
    dst_sca_ty = dst_ty.scalar
    check_dtype_support(builder, src_sca_ty)
    check_dtype_support(builder, dst_sca_ty)
...
def atom_red_typechecking_impl(ptr: tl.tensor, val: tl.tensor, mask: tl.tensor, op: str,
    ...
    # Cambricon specific.
    if builder.options.backend_name == "mlu":
        if element_ty in [tl.int1, tl.int64, tl.uint64]:
...
def dot(lhs: tl.tensor, rhs: tl.tensor, acc: tl.tensor, input_precision: Optional[str], max_num_imprecise_acc: int,
    ...
    if lhs.dtype.is_fp8() and rhs.dtype.is_fp8():
        ...
        assert lhs.dtype in (tl.int8, tl.uint8, tl.int16, tl.float16, tl.bfloat16,
                             tl.float32), f"Unsupported lhs dtype {lhs.dtype}"
        assert rhs.dtype in (tl.int8, tl.uint8, tl.int16, tl.float16, tl.bfloat16,
                             tl.float32), f"Unsupported rhs dtype {rhs.dtype}"
        assert lhs.dtype == rhs.dtype, f"Both operands must be same dtype. Got {lhs.dtype} and {rhs.dtype}"
        if lhs.dtype.is_int() or rhs.dtype.is_int():
            assert out_dtype.is_fp16() or out_dtype.is_fp32(
            ), f"When the input and weights are of int type, the output must be float or half type"
...
def scatter(dst: tl.tensor, src: tl.tensor, index: tl.tensor, axis: int, _builder: ir.builder) -> tl.tensor:
    assert index.dtype.is_int(), "index must be an integer tensor"
    assert isinstance(axis, tl.constexpr) or isinstance(axis, int), "axis must be an constant integer or int"
    ranks = [len(x.type.shape) for x in [dst, src, index]]
    assert ranks[0] == ranks[1] == ranks[2], f"source, dst and index tensors must have the same rank, {ranks}"
    rank = len(src.type.shape)
    assert len(
        index.type.shape) == rank, f"source and index tensors must have the same rank, {len(index.type.shape)}!={rank}"
    assert -rank <= axis < rank, f"scatter axis {axis} must be < source rank ({rank})"
    ...
    for d in range(rank):
        ...
        f"index.shape({d}) {index.type.shape[d]} must be == src.shape({d}) {src.type.shape[d]}"
        if d != axis:
            assert index.type.shape[d] <= dst.type.shape[d], \
            f"index.shape({d}) {index.type.shape[d]} must be <= dst.shape({d}) {dst.type.shape[d]}"

    scatter = _builder.create_scatter(dst.handle, src.handle, index.handle, axis)
    return wrap_tensor(scatter, dst.type.scalar, dst.type.shape)
...
def masked_select(input: tl.tensor, mask: tl.tensor, builder: ir.builder) -> Tuple[tl.tensor, tl.tensor]:
    mask = cast(mask, tl.int1, builder)

    input, mask = broadcast_impl_value(input, mask, builder)
    if len(input.shape) != 1:
        input = reshape(input, [input.numel], can_reorder=True, builder=builder)
        mask = reshape(mask, [mask.numel], can_reorder=True, builder=builder)

    select_val, select_num = builder.create_masked_select(input.handle, mask.handle)
    select_val_ty = tl.block_type(input.type.scalar, mask.type.shape)

    return (tl.tensor(select_val, select_val_ty), tl.tensor(select_num, tl.int32))
