...
class block_type(dtype):
    ...
    def __init__(self, element_ty: dtype, shape: List):
        ...
        assert (isinstance(shape, (list, tuple, Tuple)))
...
def _extract_slice(sl: slice, shape: tl.constexpr, builder):
    ...
    if isinstance(sl.start, tensor) and isinstance(sl.stop, tensor):
        # Slice case: data[i:i+10]
        delta = builder.compute_constant_positive_delta(sl.start.handle, sl.stop.handle)
        assert delta >= 0, f"slice delta should be deducible and be greater than 0 for data[i:i+delta]"
        start = sl.start.handle
        stop = sl.stop
...
class tensor(base_value):
    ...
    def __getitem__(self, slices, _builder=None):
        ...
        for dim, sl in enumerate(slices):
            if sl is None or isinstance(sl, (constexpr, int)):
                if sl is None or _constexpr_to_value(sl) is None:
                ...
                start, size, step = _extract_slice(sl, ret.shape[dim], _builder)
    ...
    @builtin
    def __setitem__(self, slices, value, _builder=None):
        if isinstance(slices, (slice, constexpr, tensor, builtins.slice)):
            slices = [slices]
        if isinstance(slices, tuple):
            slices = slices.values
        ret = self
    ...
    def to(self, dtype: dtype, fp_downcast_rounding: Optional[str] = None, bitcast: bool = False, _builder=None):
        ...
        semantic.check_dtype_support(_builder, self.type)
    ...
    def scatter(self, src, indices, axis) -> tensor:
        ...
    ...
    def masked_select(self, mask) -> tuple[tensor, tensor]:
        ...
...
arange.__doc__ = f"""
    ...
    :param start: Start of the interval. Must be a power of two except MLU.
    :type start: int32
    :param end: End of the interval. Must be a power of two greater than
        :code:`start` except MLU.
    :type end: int32
"""
...
@builtin
def empty(shape, dtype, _builder=None):
    """
    Returns an uninitialized tensor with the given :code:`shape` and :code:`dtype`.
    ...
    shape = _shape_check_impl(shape)
    dtype = _constexpr_to_value(dtype)
    return semantic.empty(shape, dtype, _builder)
...
@builtin
def stack(a, b, axis, _builder=None):
    """
    Stack two tensors along a new axis.

    Joins the given tensors, which must have the same shape, along the specified axis to create a new tensor.

    The `axis` parameter specifies the index of the new axis in the dimensions of the result.
    For example, if `axis=0`, the new axis will be the first dimension; if `axis=-1`, it will be the last dimension.

    Examples:
    - Given two tensors of shape (4,8), `axis=0` produces a new tensor of shape (2, 4, 8).
    - Given two tensors of shape (4,8), `axis=1` produces a new tensor of shape (4, 2, 8).
    - Given two tensors of shape (4,8), `axis=2` or `axis=-1` produces a new tensor of shape (4, 8, 2).
    - Stacking two scalars along `axis=0` or `axis=-1` produces a tensor of shape (2).

    :param a: The first input tensor.
    :type a: Tensor
    :param b: The second input tensor.
    :type b: Tensor
    :param axis: The index to add the new axis. Defaults to 0.
    :type axis: int
    """
    a = semantic.to_tensor(a, _builder)
    b = semantic.to_tensor(b, _builder)
    return semantic.stack(a, b, axis, _builder)
...
@_tensor_member_fn
@builtin
def scatter(dst, src, index, axis, _builder=None):
    """Scatter src to dst along a given dimension.
    ...
    if isinstance(src, constexpr):
        src = semantic.full(index.shape, src, index.dtype, _builder)
    elif not src.type.is_block():
        src = broadcast_to(src, index.shape, _builder=_builder)
    return semantic.scatter(dst, src, index, axis, _builder)


@_tensor_member_fn
@builtin
def masked_select(input, mask, _builder=None) -> tuple[tensor, tensor]:
    """Select elements from a tensor based on a boolean mask.

    Extracts elements from `input` where `mask` is `True`. The result is a
    1-dimensional tensor.

    :param input: the source tensor
    :type input: Tensor
    :param mask: Boolean tensor indicating which elements to select. Must be
                 broadcastable with `input`.
    :type mask: BoolTensor

    """
    input = semantic.to_tensor(input, _builder)
    mask = semantic.to_tensor(mask, _builder)
    select_val, select_num = semantic.masked_select(input, mask, _builder)
    return select_val, select_num
